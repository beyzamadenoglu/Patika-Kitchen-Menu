...
let createButton = () => {
    let buttons = 
    `<button type="button" id="All" class="btn btn-outline-dark btn-item">
     All
  </button>
  <button type="button" id="Korea" class="btn btn-outline-dark btn-item">
     Korea
  </button>
  <button type="button" id="Japan" class="btn btn-outline-dark btn-item">
     Japan
  </button>
  <button type="button" id="China" class="btn btn-outline-dark btn-item">
     China
  </button>`
...
function showFood() {
    let listFood = " "
    menu.map((event) => {
        listFood += createFoodList(event);
    });
    centerSection.innerHTML = listFood;
...
function listAll() {
    let listele = ""
    menu.map((event) => {
        listele += createFoodList(event);
    });
    centerSection.innerHTML = listele;
}

function listKorea() {
    let listele = ""
    menu.map((event) => {
        if (event.category == "Korea") {
            listele += createFoodList(event);
        }
    });
    centerSection.innerHTML = listele;
...
function createFoodList(item) {
    ...
    let menu = `<div class="menu-items col-lg-6 col-sm-12">
